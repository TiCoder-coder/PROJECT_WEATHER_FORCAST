...
"""
tuning.py - Module điều chỉnh hyperparameters cho các ML models

Mục đích:
    - Tìm kiếm hyperparameters tối ưu cho các models (XGBoost, LightGBM, RandomForest, CatBoost)
    - Hỗ trợ nhiều phương pháp tuning: GridSearch, RandomSearch, Optuna
    - Sử dụng Cross-Validation để đánh giá hiệu năng
    - Lưu kết quả tuning và best parameters
    - Tích hợp với training pipeline

Các phương pháp tuning hỗ trợ:
    1. GridSearchCV: Tìm kiếm toàn bộ grid parameters
    2. RandomizedSearchCV: Tìm kiếm ngẫu nhiên (nhanh hơn)
    3. Optuna: Tìm kiếm thông minh (TPE sampler, pruning)
...
# Hyperparameter search spaces cho các models
...
class HyperparameterTuner:
    """
    Lớp điều chỉnh hyperparameters cho ML models.
    
    Hỗ trợ:
    - GridSearchCV: Tìm kiếm toàn bộ (chậm)
    - RandomizedSearchCV: Tìm kiếm ngẫu nhiên (nhanh hơn)
    - Optuna: Tìm kiếm thông minh (nhanh nhất + tốt nhất)
    """
