...
class TaskType(Enum):
    """Enum mô tả loại bài toán ML."""
    CLASSIFICATION = "classification"
    REGRESSION = "regression"


class ModelStatus(Enum):
    """Enum mô tả trạng thái model trong vòng đời."""
    UNTRAINED = "untrained"  # chưa train / chưa load
    TRAINED = "trained"      # đã train hoặc load thành công
    FAILED = "failed"        # train thất bại
...
class TrainingResult:
    """
    Kết quả train trả về cho caller:
    - success: trạng thái thành công / thất bại
    - metrics: dict metrics (rmse/mae/r2 hoặc accuracy/f1...)
    - training_time: thời gian train (giây)
    - n_samples / n_features: kích thước data train thực tế
    - feature_names: danh sách cột feature sau preprocess
    - feature_importances: tầm quan trọng feature (LightGBM)
    - best_iteration: best_iteration_ nếu dùng early stopping
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Chuyển dataclass sang dict để lưu metadata/log/export."""
        return asdict(self)
...
class PredictionResult:
    """
    Kết quả predict:
    - predictions: ndarray dự đoán (regression: float, classification: label đã decode)
    - probabilities: xác suất (chỉ có khi classification + return_proba=True)
    - prediction_time: thời gian suy luận (giây)
    """
    ...
    def to_list(self) -> List[Any]:
        """Tiện cho JSON response / serialize."""
...
class WeatherLightGBM:
    ...
    def __init__(
        ...
        # Xác định task: regression / classification# ----------------------------- LIGHTGBM MODEL - GRADIENT BOOSTING (GBDT) -----------------------------------------------------------
"""
Weather_Forcast_App.Machine_learning_model.Models.LightGBM_Model
