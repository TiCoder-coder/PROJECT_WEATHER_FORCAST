...
class SQLiteManager:
        ...
        try:
            ...
            for station in stations:
                self.cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    station.get('station_id', f"ST{hash(station.get('station_name', '')) % 1000000:06d}"),
    ...
    def insert_vrain_data(self, vrain_data):
        ...
        try:
            ...
            for data in vrain_data:
                ...
                ''', (
                    data.get('station_id', f"VR{hash(data.get('station_name', '')) % 1000000:06d}"),
            ...
            self.conn.commit()
            logging.info(f"✅ Đã chèn {inserted_count} bản ghi dữ liệu Vrain vào database")
            return inserted_count
    ...
    def get_province_rainfall_summary(self, date=None):
        ...
        try:
            ...
            results = self.cursor.fetchall()
            columns = ['province_name', 'data_points', 'avg_rainfall_1h', 'avg_rainfall_24h', 
                      'total_rainfall_24h', 'max_rainfall_1h', 'min_rainfall_1h']
    ...
    def get_vrain_province_summary(self):
        ...
        try:
            ...
            results = self.cursor.fetchall()
            columns = ['province_name', 'station_count', 'avg_rainfall', 'max_rainfall', 'min_rainfall', 'total_rainfall']
...
class VrainScraper:
    ...
    def __init__(self):
        ...
        self.district_keywords = [
            ...
            'Đống Đa', 'Ba Đình', 'Hoàn Kiếm', 'Hai Bà Trưng', 'Cầu Giấy',
            'Thanh Xuân', 'Hoàng Mai', 'Long Biên', 'Tây Hồ', 'Bắc Từ Liêm'
        ]
    ...
    def extract_stations_from_html(self, html_content: str) -> List[Dict]:
        ...
        try:
            ...
            tables = soup.find_all('table', class_=re.compile(r'(station|data|rainfall)', re.I))
                ...
                for row in rows:
                    ...
                    if len(cols) >= 2:  # Có ít nhất 2 cột
                        ...
                        if station_name and len(station_name) > 2:
                            ...
                            if len(cols) > 3:
                                station_data['station_code'] = cols[1].get_text(strip=True) if len(cols) > 1 else ''
                                try:
                                    station_data['latitude'] = float(cols[2].get_text(strip=True)) if len(cols) > 2 else 0
                                    station_data['longitude'] = float(cols[3].get_text(strip=True)) if len(cols) > 3 else 0
                                except:
                                    pass
            ...
            station_divs = soup.find_all(['div', 'span', 'li'], 
                                        class_=re.compile(r'(station|trạm|point)', re.I))
    ...
    def extract_real_data_from_html(self, html_content: str) -> List[Dict]:
        ...
        try:
            ...
            tables = soup.find_all('table', class_=re.compile(r'(table|data|rainfall|station)', re.I))
            
            for table in tables:
                rows = table.find_all('tr')
                for row in rows:
                    cols = row.find_all(['td', 'th'])
                    if len(cols) >= 3:  # Có ít nhất 3 cột: Tên trạm, Lượng mưa, Thời gian
                        try:
                            station_name = cols[0].get_text(strip=True)
                            rainfall_text = cols[1].get_text(strip=True)
                            time_text = cols[2].get_text(strip=True) if len(cols) > 2 else ''
                            ...
                            if rainfall_match:
                                ...
                                all_data.append({
                                    ...
                                    'rainfall_unit': 'mm',
                                    'rainfall_description': self._get_rainfall_description(rainfall_value),
            ...
            if not all_data:
                data_sections = soup.find_all(['div', 'section'], 
                                            attrs={'class': re.compile(r'(station|rainfall|data|measurement)', re.I)})
                        ...
                        for pattern in patterns:
                            ...
                            if match:
                                ...
                                all_data.append({
                                    ...
                                    'rainfall_unit': 'mm',
                                    'rainfall_description': self._get_rainfall_description(rainfall_value),
                                    'measurement_time': self._parse_time(match.group(4) if len(match.groups()) > 3 and match.group(4) else ''),
                                    'data_source': 'vrain.vn (real)'
                                })
                                break
    ...
    def _parse_time(self, time_str: str) -> str:
        ...
        try:
            ...
            if re.match(r'\d{1,2}:\d{2}', time_str):
                ...
                return datetime.combine(today, time_obj.time()).strftime('%Y-%m-%d %H:%M:%S')
            ...
            # Format: "HH:MM DD/MM"
                ...
                if len(day_month) == 2:
                    dt_obj = datetime.strptime(f"{today.year}-{day_month[1]}-{day_month[0]} {hour_min}", 
                                             "%Y-%m-%d %H:%M")
            ...
            # Format: "DD/MM HH:MM"
                ...
                day_month = date_part.split('/')
                if len(day_month) == 2:
                    dt_obj = datetime.strptime(f"{today.year}-{day_month[1]}-{day_month[0]} {time_part}", 
                                             "%Y-%m-%d %H:%M")
                    return dt_obj.strftime('%Y-%m-%d %H:%M:%S')
        ...
        try:
            if isinstance(json_data, dict):
                ...
                if 'stations' in json_data:
                    for station in json_data['stations']:
                        if isinstance(station, dict):
                            province_name = self._identify_province(station.get('name', ''))
                            ...
                            processed_data.append({
                                ...
                                'rainfall_description': self._get_rainfall_description(float(station.get('rainfall', 0))),
                                'measurement_time': self._parse_time(station.get('time', '')),
                ...
                # Cấu trúc 2: {"data": [{"station": "...", "value": "...", "timestamp": "..."}]}
                    ...
                    for item in json_data['data']:
                        if isinstance(item, dict):
                            province_name = self._identify_province(item.get('station', ''))
                            ...
                            processed_data.append({
                                ...
                                'rainfall_unit': 'mm',
                                'rainfall_description': self._get_rainfall_description(float(item.get('value', 0))),
                                'measurement_time': self._parse_time(item.get('timestamp', '')),
                                'data_source': 'vrain.vn (JSON)'
                            })
                ...
                for item in json_data:
                    if isinstance(item, dict):
                        province_name = self._identify_province(item.get('station', item.get('name', '')))
                        district = self._identify_district(item.get('station', item.get('name', '')))
                        
                        processed_data.append({
                            'province_name': province_name,
                            'station_name': item.get('station', item.get('name', '')),
                            'district': district,
                            'rainfall_value': float(item.get('rainfall', item.get('value', 0))),
                            'rainfall_unit': 'mm',
                            'rainfall_description': self._get_rainfall_description(float(item.get('rainfall', item.get('value', 0)))),
                            'measurement_time': self._parse_time(item.get('time', item.get('timestamp', ''))),
                            'data_source': 'vrain.vn (JSON)'
                        })
                        ...
                        if response.status_code == 200:
                            ...
                            if 'application/json' in content_type:
                                ...
                                if stations:
                                    ...
                                    logging.info(f"✅ Tìm thấy {len(stations)} trạm từ API: {endpoint}")
                                    break
                except:
                    continue
                ...
                if 'stations' in json_data:
                    for station in json_data['stations']:
                        stations.append({
                            ...
                            'province_name': self._identify_province(station.get('name', '')),
                            'district': self._identify_district(station.get('name', '')),
                            'latitude': station.get('lat', station.get('latitude', 0)),
                            'longitude': station.get('lon', station.get('longitude', 0)),
                    ...
                    for item in json_data['data']:
                        stations.append({
                            'station_name': item.get('name', item.get('station', '')),
                            'station_code': item.get('code', ''),
                            'province_name': self._identify_province(item.get('name', item.get('station', ''))),
                            'district': self._identify_district(item.get('name', item.get('station', ''))),
    ...
    def generate_sample_stations(self) -> List[Dict]:
        ...
        for province_name in self.province_mapping.keys():
            ...
            for i in range(num_stations):
                # Tạo tên trạm
                station_types = ['Trạm', 'Đài', 'Trạm đo', 'Trạm quan trắc', 'Trạm khí tượng']
                station_type = random.choice(station_types)
                
                # Tên địa danh phổ biến
                location_names = ['Trung tâm', 'Bắc', 'Nam', 'Đông', 'Tây', 
                                 'Trung tâm TP', 'Ngoại thành', 'Ven biển', 'Vùng núi']
                ...
                # Tạo huyện
                districts = ['Quận 1', 'Quận 2', 'Quận 3', 'Huyện A', 'Huyện B', 'Thành phố', 'Thị xã']
    ...
    def enrich_station_data(self, stations: List[Dict]) -> List[Dict]:
        ...
        for station in stations:
            ...
            required_fields = ['station_name', 'province_name', 'district', 
                              'latitude', 'longitude', 'elevation', 'station_type', 'data_source']
        ...
        try:
            ...
            for station in stations:
                try:
                    ...
                    logging.debug(f"Lỗi xử lý trạm {station.get('station_name', '')}: {e}")
                    continue
    ...
    def _generate_realistic_rainfall(self, station: Dict) -> float:
        ...
        # Xác định vùng
        region = 'Miền Bắc'
        if province_name in ['TP Hồ Chí Minh', 'Bình Dương', 'Đồng Nai', 'Bà Rịa - Vũng Tàu', 
                           'Long An', 'Tiền Giang', 'Bến Tre', 'Trà Vinh', 'Vĩnh Long', 
                           'Đồng Tháp', 'An Giang', 'Kiên Giang', 'Cần Thơ', 'Hậu Giang', 
                           'Sóc Trăng', 'Bạc Liêu', 'Cà Mau']:
            region = 'Miền Nam'
        elif province_name in ['Thanh Hóa', 'Nghệ An', 'Hà Tĩnh', 'Quảng Bình', 'Quảng Trị',
        ...
        for province_name in self.province_mapping.keys():
            ...
            for i in range(num_stations):
                ...
                districts = ['Quận 1', 'Quận 2', 'Quận 3', 'Quận 4', 'Quận 5',
                           'Huyện A', 'Huyện B', 'Huyện C', 'Thành phố', 'Thị xã']
                district = random.choice(districts) if random.random() > 0.4 else ""
                
                # Tạo lượng mưa thực tế
                rainfall_value = self._generate_realistic_rainfall({'province_name': province_name})
...
class VietnamWeatherCrawler:
    ...
    def load_all_vietnam_provinces(self):
        ...
        provinces = [
            # Miền Bắc (28 tỉnh)
            {"province_id": "01", "province_name": "Hà Nội", "region": "Đồng bằng sông Hồng", "latitude": 21.0285, "longitude": 105.8542, "total_districts": 30},
            {"province_id": "02", "province_name": "Hải Phòng", "region": "Đồng bằng sông Hồng", "latitude": 20.8449, "longitude": 106.6881, "total_districts": 15},
            {"province_id": "03", "province_name": "Quảng Ninh", "region": "Đông Bắc Bộ", "latitude": 21.0064, "longitude": 107.2925, "total_districts": 13},
            {"province_id": "04", "province_name": "Bắc Giang", "region": "Đông Bắc Bộ", "latitude": 21.2814, "longitude": 106.1975, "total_districts": 10},
            {"province_id": "05", "province_name": "Bắc Ninh", "region": "Đồng bằng sông Hồng", "latitude": 21.1214, "longitude": 106.1111, "total_districts": 8},
            {"province_id": "06", "province_name": "Hải Dương", "region": "Đồng bằng sông Hồng", "latitude": 20.9397, "longitude": 106.3308, "total_districts": 12},
            {"province_id": "07", "province_name": "Hưng Yên", "region": "Đồng bằng sông Hồng", "latitude": 20.6461, "longitude": 106.0511, "total_districts": 10},
            {"province_id": "08", "province_name": "Thái Bình", "region": "Đồng bằng sông Hồng", "latitude": 20.4461, "longitude": 106.3366, "total_districts": 8},
            {"province_id": "09", "province_name": "Hà Nam", "region": "Đồng bằng sông Hồng", "latitude": 20.5411, "longitude": 105.9139, "total_districts": 6},
            {"province_id": "10", "province_name": "Nam Định", "region": "Đồng bằng sông Hồng", "latitude": 20.4200, "longitude": 106.1683, "total_districts": 10},
            {"province_id": "11", "province_name": "Ninh Bình", "region": "Đồng bằng sông Hồng", "latitude": 20.2539, "longitude": 105.9750, "total_districts": 8},
            {"province_id": "12", "province_name": "Vĩnh Phúc", "region": "Đồng bằng sông Hồng", "latitude": 21.3089, "longitude": 105.6044, "total_districts": 9},
            {"province_id": "13", "province_name": "Phú Thọ", "region": "Đông Bắc Bộ", "latitude": 21.3000, "longitude": 105.4333, "total_districts": 13},
            {"province_id": "14", "province_name": "Thái Nguyên", "region": "Đông Bắc Bộ", "latitude": 21.5928, "longitude": 105.8442, "total_districts": 9},
            {"province_id": "15", "province_name": "Lào Cai", "region": "Tây Bắc", "latitude": 22.4833, "longitude": 103.9500, "total_districts": 9},
            {"province_id": "16", "province_name": "Yên Bái", "region": "Tây Bắc", "latitude": 21.7000, "longitude": 104.8667, "total_districts": 9},
            {"province_id": "17", "province_name": "Sơn La", "region": "Tây Bắc", "latitude": 21.3256, "longitude": 103.9189, "total_districts": 12},
            {"province_id": "18", "province_name": "Điện Biên", "region": "Tây Bắc", "latitude": 21.3833, "longitude": 103.0167, "total_districts": 10},
            {"province_id": "19", "province_name": "Lai Châu", "region": "Tây Bắc", "latitude": 22.4000, "longitude": 103.4500, "total_districts": 8},
            {"province_id": "20", "province_name": "Hòa Bình", "region": "Tây Bắc", "latitude": 20.8133, "longitude": 105.3383, "total_districts": 11},
            {"province_id": "21", "province_name": "Cao Bằng", "region": "Đông Bắc Bộ", "latitude": 22.6667, "longitude": 106.2500, "total_districts": 10},
            {"province_id": "22", "province_name": "Bắc Kạn", "region": "Đông Bắc Bộ", "latitude": 22.1500, "longitude": 105.8333, "total_districts": 8},
            {"province_id": "23", "province_name": "Lạng Sơn", "region": "Đông Bắc Bộ", "latitude": 21.8478, "longitude": 106.7578, "total_districts": 11},
            {"province_id": "24", "province_name": "Tuyên Quang", "region": "Đông Bắc Bộ", "latitude": 21.8181, "longitude": 105.2144, "total_districts": 7},
            {"province_id": "25", "province_name": "Hà Giang", "region": "Đông Bắc Bộ", "latitude": 22.8233, "longitude": 104.9836, "total_districts": 11},
            
            # Miền Trung (19 tỉnh)
            {"province_id": "26", "province_name": "Thanh Hóa", "region": "Bắc Trung Bộ", "latitude": 19.8000, "longitude": 105.7667, "total_districts": 27},
            {"province_id": "27", "province_name": "Nghệ An", "region": "Bắc Trung Bộ", "latitude": 18.6733, "longitude": 105.6811, "total_districts": 21},
            {"province_id": "28", "province_name": "Hà Tĩnh", "region": "Bắc Trung Bộ", "latitude": 18.3333, "longitude": 105.9000, "total_districts": 13},
            {"province_id": "29", "province_name": "Quảng Bình", "region": "Bắc Trung Bộ", "latitude": 17.4687, "longitude": 106.6227, "total_districts": 8},
            {"province_id": "30", "province_name": "Quảng Trị", "region": "Bắc Trung Bộ", "latitude": 16.8160, "longitude": 107.1000, "total_districts": 10},
            {"province_id": "31", "province_name": "Thừa Thiên Huế", "region": "Bắc Trung Bộ", "latitude": 16.4637, "longitude": 107.5909, "total_districts": 9},
            {"province_id": "32", "province_name": "Đà Nẵng", "region": "Nam Trung Bộ", "latitude": 16.0592, "longitude": 108.2208, "total_districts": 8},
            {"province_id": "33", "province_name": "Quảng Nam", "region": "Nam Trung Bộ", "latitude": 15.5667, "longitude": 108.4833, "total_districts": 18},
            {"province_id": "34", "province_name": "Quảng Ngãi", "region": "Nam Trung Bộ", "latitude": 15.1167, "longitude": 108.8000, "total_districts": 14},
            {"province_id": "35", "province_name": "Bình Định", "region": "Nam Trung Bộ", "latitude": 13.7667, "longitude": 109.2333, "total_districts": 11},
            {"province_id": "36", "province_name": "Phú Yên", "region": "Nam Trung Bộ", "latitude": 13.0833, "longitude": 109.3000, "total_districts": 9},
            {"province_id": "37", "province_name": "Khánh Hòa", "region": "Nam Trung Bộ", "latitude": 12.2500, "longitude": 109.1833, "total_districts": 9},
            {"province_id": "38", "province_name": "Ninh Thuận", "region": "Nam Trung Bộ", "latitude": 11.5667, "longitude": 108.9833, "total_districts": 7},
            {"province_id": "39", "province_name": "Bình Thuận", "region": "Nam Trung Bộ", "latitude": 10.9333, "longitude": 108.1000, "total_districts": 10},
            {"province_id": "40", "province_name": "Kon Tum", "region": "Tây Nguyên", "latitude": 14.3833, "longitude": 107.9833, "total_districts": 10},
            {"province_id": "41", "province_name": "Gia Lai", "region": "Tây Nguyên", "latitude": 13.9833, "longitude": 108.0000, "total_districts": 17},
            {"province_id": "42", "province_name": "Đắk Lắk", "region": "Tây Nguyên", "latitude": 12.6662, "longitude": 108.0382, "total_districts": 15},
            {"province_id": "43", "province_name": "Đắk Nông", "region": "Tây Nguyên", "latitude": 12.0042, "longitude": 107.6907, "total_districts": 8},
            {"province_id": "44", "province_name": "Lâm Đồng", "region": "Tây Nguyên", "latitude": 11.9404, "longitude": 108.4587, "total_districts": 12},
            
            # Miền Nam (16 tỉnh)
            {"province_id": "45", "province_name": "TP Hồ Chí Minh", "region": "Đông Nam Bộ", "latitude": 10.7757, "longitude": 106.7004, "total_districts": 24},
            {"province_id": "46", "province_name": "Bình Dương", "region": "Đông Nam Bộ", "latitude": 10.9804, "longitude": 106.6519, "total_districts": 9},
            {"province_id": "47", "province_name": "Đồng Nai", "region": "Đông Nam Bộ", "latitude": 10.9574, "longitude": 106.8429, "total_districts": 11},
            {"province_id": "48", "province_name": "Bà Rịa - Vũng Tàu", "region": "Đông Nam Bộ", "latitude": 10.3460, "longitude": 107.0843, "total_districts": 8},
            {"province_id": "49", "province_name": "Bình Phước", "region": "Đông Nam Bộ", "latitude": 11.5349, "longitude": 106.8823, "total_districts": 11},
            {"province_id": "50", "province_name": "Tây Ninh", "region": "Đông Nam Bộ", "latitude": 11.3131, "longitude": 106.0963, "total_districts": 9},
            {"province_id": "51", "province_name": "Long An", "region": "Đồng bằng sông Cửu Long", "latitude": 10.5333, "longitude": 106.4167, "total_districts": 15},
            {"province_id": "52", "province_name": "Tiền Giang", "region": "Đồng bằng sông Cửu Long", "latitude": 10.3500, "longitude": 106.3500, "total_districts": 11},
            {"province_id": "53", "province_name": "Bến Tre", "region": "Đồng bằng sông Cửu Long", "latitude": 10.2333, "longitude": 106.3833, "total_districts": 9},
            {"province_id": "54", "province_name": "Trà Vinh", "region": "Đồng bằng sông Cửu Long", "latitude": 9.9347, "longitude": 106.3453, "total_districts": 9},
            {"province_id": "55", "province_name": "Vĩnh Long", "region": "Đồng bằng sông Cửu Long", "latitude": 10.2500, "longitude": 105.9667, "total_districts": 8},
            {"province_id": "56", "province_name": "Đồng Tháp", "region": "Đồng bằng sông Cửu Long", "latitude": 10.4500, "longitude": 105.6333, "total_districts": 12},
            {"province_id": "57", "province_name": "An Giang", "region": "Đồng bằng sông Cửu Long", "latitude": 10.3865, "longitude": 105.4351, "total_districts": 11},
            {"province_id": "58", "province_name": "Kiên Giang", "region": "Đồng bằng sông Cửu Long", "latitude": 10.0317, "longitude": 105.0809, "total_districts": 15},
            {"province_id": "59", "province_name": "Cần Thơ", "region": "Đồng bằng sông Cửu Long", "latitude": 10.0452, "longitude": 105.7469, "total_districts": 9},
            {"province_id": "60", "province_name": "Hậu Giang", "region": "Đồng bằng sông Cửu Long", "latitude": 9.7833, "longitude": 105.4667, "total_districts": 7},
            {"province_id": "61", "province_name": "Sóc Trăng", "region": "Đồng bằng sông Cửu Long", "latitude": 9.6025, "longitude": 105.9739, "total_districts": 11},
            {"province_id": "62", "province_name": "Bạc Liêu", "region": "Đồng bằng sông Cửu Long", "latitude": 9.2833, "longitude": 105.7167, "total_districts": 7},
            {"province_id": "63", "province_name": "Cà Mau", "region": "Đồng bằng sông Cửu Long", "latitude": 9.1769, "longitude": 105.1521, "total_districts": 9}
        ]
        ...
        try:
            ...
            for station in stations_data:
                ...
                if not station_rain_data:
                    station_rain_data = {
                        'station_name': station['station_name'],
                        'rainfall_value': self.vrain_scraper._generate_realistic_rainfall(station),
                        'rainfall_unit': 'mm',
                        'measurement_time': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
            ...
            combined_data.sort(key=lambda x: (x.get('province_name', ''), x.get('station_name', '')))
            
            # Chuyển đổi sang định dạng weather_data
            weather_data = self.convert_vrain_to_weather_format(combined_data)
            
            logging.info(f"✅ Đã thu thập {len(combined_data)} trạm từ {len(set(d['province_name'] for d in combined_data))} tỉnh")
            ...
            for data in vrain_data:
                ...
                if province_info:
                    weather_data = {
                        ...
                        'timestamp': data.get('measurement_time', datetime.now().strftime('%Y-%m-%d %H:%M:%S')),
                        'data_source': 'vrain.vn (tổng hợp)',
                        'data_quality': 'high',
                        ...
                        'weather_description': f'Mưa {rainfall_value:.1f}mm' if rainfall_value > 0 else 'Trời quang',
                        'weather_icon': '10d' if rainfall_value > 0 else '01d'
                    }
                    
                    weather_data_list.append(weather_data)
            
            logging.info(f"✅ Đã chuyển đổi {len(weather_data_list)} bản ghi sang định dạng thời tiết")
    ...
    def save_comprehensive_data(self, data):
        ...
        try:
            ...
            logging.info(f"💾 Đã lưu {stations_count} trạm, {vrain_count} bản ghi Vrain, {weather_count} bản ghi thời tiết")
    ...
    def save_comprehensive_excel(self, combined_data, output_dir="output"):
        """Lưu dữ liệu toàn diện ra file Excel"""
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        excel_file = os.path.join(output_dir, f"vrain_comprehensive_data_{timestamp}.xlsx")
        ...
        title_cell = ws_stations.cell(row=1, column=1, 
                                    value=f"DANH SÁCH TRẠM QUAN TRẮC THEO TỈNH THÀNH")
        title_cell.font = Font(bold=True, size=16, color="366092")
        title_cell.alignment = Alignment(horizontal="center")
        
        ws_stations.merge_cells('A2:H2')
        time_cell = ws_stations.cell(row=2, column=1, 
        ...
        # Header
        headers = ['STT', 'Tỉnh/TP', 'Tên Trạm', 'Huyện/Quận', 'Vĩ độ', 'Kinh độ', 
                  'Độ cao (m)', 'Loại trạm', 'Nguồn dữ liệu']
        ...
        for idx, data in enumerate(sorted_data, start=1):
            ...
            if province_name != current_province:
                if current_province != "":
                    ...
                    ws_stations.cell(row=row_idx, column=2, value=f"Tổng số trạm {current_province}:")
                    ws_stations.cell(row=row_idx, column=3, value=idx - province_start_row)
                    for c in range(1, len(headers) + 1):
                ...
                province_cell = ws_stations.cell(row=row_idx, column=1, value=f"TỈNH: {province_name}")
                province_cell.font = Font(bold=True, size=12, color="FFFFFF")
                province_cell.fill = PatternFill(start_color="00B050", end_color="00B050", fill_type="solid")
        ...
        title_cell = ws_rainfall.cell(row=1, column=1, 
                                    value="DỮ LIỆU LƯỢNG MƯA THEO TRẠM")
        title_cell.font = Font(bold=True, size=14, color="FF6600")
        title_cell.alignment = Alignment(horizontal="center")
        
        # Header
        rain_headers = ['STT', 'Tỉnh/TP', 'Tên Trạm', 'Huyện/Quận', 'Lượng Mưa (mm)', 
                       'Mô tả', 'Thời gian đo', 'Nguồn dữ liệu']
        ...
        title_cell = ws_stats.cell(row=1, column=1, 
                                 value="THỐNG KÊ TRẠM VÀ LƯỢNG MƯA THEO TỈNH")
        title_cell.font = Font(bold=True, size=14, color="800080")
        title_cell.alignment = Alignment(horizontal="center")
        
        # Header
        stats_headers = ['STT', 'Tỉnh/TP', 'Vùng', 'Số Trạm', 'Lượng Mưa TB (mm)', 
                        'Lượng Mưa Max (mm)', 'Lượng Mưa Min (mm)', 'Trạng thái']
        ...
        for data in combined_data:
            ...
            province_stats[province_name]['stations'].append(data.get('station_name', ''))
            province_stats[province_name]['rainfall_values'].append(data.get('rainfall_value', 0))
            
            # Tìm vùng cho tỉnh
            if not province_stats[province_name]['region']:
                for province in self.provinces_data:
                    if province['province_name'] == province_name:
                        province_stats[province_name]['region'] = province.get('region', 'Khác')
                        break
        
        # Dữ liệu thống kê
        row_idx = 4
        for idx, (province_name, stats) in enumerate(sorted(province_stats.items()), start=1):
        ...
        title_cell = ws_summary.cell(row=1, column=1, 
                                   value="TỔNG QUAN HỆ THỐNG TRẠM QUAN TRẮC")
        ...
        summary_data = [
            ...
            ["Tỉnh có nhiều trạm nhất", max(province_stats.items(), key=lambda x: len(x[1]['stations']))[0] if province_stats else "N/A"],
            ["Tỉnh có lượng mưa TB cao nhất", max(province_stats.items(), key=lambda x: sum(x[1]['rainfall_values'])/len(x[1]['rainfall_values']) if x[1]['rainfall_values'] else 0)[0] if province_stats else "N/A"],
            ["Tổng lượng mưa TB toàn quốc", round(sum(sum(stats['rainfall_values']) for stats in province_stats.values()) / sum(len(stats['rainfall_values']) for stats in province_stats.values()), 2) if province_stats else 0],
            ["Số trạm có mưa (>0 mm)", sum(1 for data in combined_data if data.get('rainfall_value', 0) > 0)],
            ["Số trạm không mưa", sum(1 for data in combined_data if data.get('rainfall_value', 0) == 0)],
        ]
    ...
    try:
        ...
        if combined_data:
            ...
            for data in combined_data:
                ...
                province_summary[province_name]['stations'].append(data.get('station_name', ''))
                province_summary[province_name]['rainfall_values'].append(data.get('rainfall_value', 0))
            ...
            for province_name, stats in sorted(province_summary.items()):
                ...
                logging.info(f"   {status} {province_name}: {station_count} trạm, {avg_rainfall:.1f} mm TB")
            
            # Top 5 tỉnh có nhiều trạm nhất
            sorted_by_stations = sorted(province_summary.items(), 
            ...
            sorted_by_rainfall = sorted(province_summary.items(), 
                                      key=lambda x: sum(x[1]['rainfall_values'])/len(x[1]['rainfall_values']) if x[1]['rainfall_values'] else 0, 
                                      reverse=True)[:5]
            
            logging.info("🌧️ TOP 5 TỈNH CÓ LƯỢNG MƯA CAO NHẤT:")
            for i, (province, stats) in enumerate(sorted_by_rainfall, 1):
                avg_rain = sum(stats['rainfall_values'])/len(stats['rainfall_values']) if stats['rainfall_values'] else 0
