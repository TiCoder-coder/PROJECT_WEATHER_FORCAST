...
def _clean_dataframe(df: pd.DataFrame, job_id: str, technique: str = "all"):
    ...
    if run_all or technique == "format":
        ...
        for c in obj_cols:
            # Dùng apply() thuần Python để tránh segfault trên pandas 2.3.x + numpy 2.4.x
            # (bug: vectorized .str.replace / pd.to_numeric crash khi object column chứa mixed str+NaN)
            try:
                vals = df[c].tolist()
                nums = []
                for v in vals:
                    ...
                    try:
                        ...
                        nums.append(np.nan)
                s = pd.Series(nums, index=df.index, dtype=float)
                n_valid = int(s.notna().sum())
                n_threshold = max(5, int(0.6 * sum(1 for v in vals if v is not None and not (isinstance(v, float) and np.isnan(v)))))
                if n_valid >= n_threshold:
                    df[c] = s
            except Exception:
                pass
