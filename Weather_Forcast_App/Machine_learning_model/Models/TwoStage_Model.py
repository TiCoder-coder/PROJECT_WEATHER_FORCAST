...
class WeatherTwoStageModel:
    ...
    def predict(
        ...
        Returns PredictionResult với:
          - predictions: np.ndarray shape (n,) — lượng mưa dự đoán (mm)
          - probabilities: np.ndarray shape (n,) — xác suất có mưa từ Stage 1
        """
        ...
        # Tính clf_proba một lần rồi tái dùng cho cả _combine và probabilities
        clf_proba = self.clf.predict_proba(X_arr)[:, 1]
        preds = self._combine(X_arr, clf_proba_override=clf_proba)

        return PredictionResult(
            predictions=preds,
            probabilities=clf_proba,   # xác suất có mưa (P(rain)) từ classifier
